...
*/
var rawVaporSlotKey = Symbol(`rawVaporSlot`);
/**
...
* vapor interop.
*/
var vdomSlotFallbackKey = Symbol(`vdomSlotFallback`);
/**
...
var parseAdoptTarget;
var isComment = (node, data) => node.nodeType === 8 && node.data === data;
var markerlessHydrationContainer = null;
...
var innermostFragmentClaim = null;
/**
...
* functions).
*/
var interopSlotsKey = Symbol(`interopSlots`);
/**
...
function getAttrFromRawProps(rawProps, key) {
	...
	if (merged && merged.length) return merged.reverse();
}
...
var isAsyncComponentEnabled = false;
...
var currentSlotHydrationSession = null;
...
*/
var INERT_PENDING_SLOT_CONTENT = {
	finish: NOOP,
	settle: NOOP
};
...
function isReusableAnchorCandidate(node, frag) {
	return !!node && (isComment(node, "") || isComment(node, "]") || frag !== void 0 && (frag.__vf & 256 && isComment(node, "v-if") || frag.anchorLabel !== void 0 && isComment(node, frag.anchorLabel)));
}
...
*/
var stampedTemplates = /* @__PURE__ */ new WeakMap();
...
var publishInteropScopeIds = null;
...
var DynamicFragment = class extends RenderContextFragment {
	...
	renderBranch(render, transition, parent, key, noScope, notifyUpdated, removePrevious) {
		...
		const u = this.u;
		if (notifyUpdated && u) u.forEach((hook) => hook(this.nodes));
	}
};
var SlotFragment = class extends DynamicFragment {
	constructor(flags = 0, adoptAnchor) {
		super(1156, "slot", false, false, false, void 0, adoptAnchor);
		this.disposed = false;
		...
		try {
			...
			if (isHydrating$1 && isAsyncComponentEnabled && isAsyncWrapper(instance) && component.__asyncHydrate) {
				...
				if (isHydrating$1 && isSuspenseEnabled && instance.suspense && instance.asyncDep) {
					const pendingBlock = [];
					let node = hydrationClaim && hydrationClaim.start || hydrationCursor.start;
...
function defineVaporAsyncComponent(source) {
	...
	return /* @__PURE__ */ defineVaporComponent({
		...
		__asyncHydrate(el, instance, hydrate) {
			...
			const prev = setCurrentInstance(instance);
			try {
				const frag = new DynamicFragment(0, "async component", false, false);
				frag.nodes = nodes;
				instance.block = frag;
			} finally {
				restoreCurrentInstance(prev);
		...
		setup() {
			...
			if (isHydrating$1 && isDynamicFragment(placeholder)) {
				...
				frag.nodes = EMPTY_BLOCK;
				locateHydrationNode();
			} else frag = new DynamicFragment(0, "async component");
			let resolvedComp = getResolvedComp();
			if (resolvedComp) {
...
function resolveTransitionHooks(block, props, state, instance) {
...
function isPersistedRoot(block) {
	while (block) {
		...
		else if (isFragment(block) && (isSlotFragment(block) || !(isDynamicFragment(block) || isForFragment(block)))) block = block.nodes;
		else return false;
	}
	return false;
}
...
function renderVDOMSlot(internals, slotsRef, name, props, parentComponent, options = EMPTY_OBJ) {
	...
	function renderContent() {
		notifyBeforeUpdate();
		runWithFragmentCtxOnly(frag, () => withSlotBoundary(contentBoundary, () => {
...
var interopRootSyncFragmentMap = /* @__PURE__ */ new WeakMap();
...
var TeleportFragment = class extends RenderContextFragment {
	...
	initChildren() {
		...
		try {
			...
			renderEffect(() => runWithFragmentCtxOnly(this, () => {
	...
	queueTargetUpdate() {
		...
		queuePostRenderEffect(this.mountToTargetJob, void 0, this.parentSuspense !== void 0 ? this.parentSuspense : isSuspenseEnabled && this.renderSuspense || null);
	}
	mountToTarget() {
		...
		try {
			...
			if (claim.start) {
				...
				if (!newLength && afterRows && isComment(afterRows, "")) parentAnchor = claimAnchor(afterRows);
					...
					parentAnchor = claimAnchor(/* @__PURE__ */ createComment("for"));
				...
				if (!isComment(close, "]")) throw new Error(`v-for fragment anchor node was not found. this is likely a Vue internal bug.`);
	...
	const renderFn = () => {
		...
		frag.update(() => {
			...
			if (isInteropEnabled && appContext.vdom && isVNode(value)) {
				...
				if (isHydrating$1) {
					locateHydrationNode(shouldConsumeFragmentStart(value) ? createFragmentClaim() : void 0);
					frag.hydrate();
				}
				return frag;
			}
...
var trackedTransitionGroupOwners = /* @__PURE__ */ new WeakSet();
var decorate = (t) => {
	delete t.props.mode;
	return t;
};
...
function applyGroupTransitionHooks(block, props, state, instance, updateHooks) {
	...
	for (let i = 0; i < children.length; i++) {
		...
		if (isValidTransitionBlock(child)) {
			if (getTransitionKey(child) != null) child.$transition = resolveTransitionHooks(child, props, state, instance);
...
//#endregion
export { setDOMProp as $, customRef as $a, resolveDirective as $i, createHydrationRenderer as $n, isMismatchAllowed as $r, useHost as $t, createVaporApp as A, useSlots as Aa, onUnmounted as Ai, Static as An, normalizeProps as Ao, getInheritedScopeIds as Ar, defineSSRCustomElement as At, insert as B, watch as Ba, provide as Bi, callWithAsyncErrorHandling as Bn, initCustomFormatter as Br, normalizeContainer as Bt, createInvoker as C, unregisterHMR as Ca, onBeforeUpdate as Ci, ErrorTypeStrings as Cn, toValue as Co, filterModelListeners as Cr, VueElementBase as Ct, createSlot as D, useInstanceOption as Da, onRenderTracked as Di, MoveType as Dn, capitalize as Do, getContainerType as Dr, createApp as Dt, createSelector as E, useId as Ea, onMounted as Ei, MismatchTypes as En, camelize as Eo, getComponentName as Er, callPendingCbs as Et, defineVaporSSRCustomElement as F, vdomSlotFallbackKey as Fa, performTransitionLeave as Fi, activate as Fn, hasInjectionContext as Fr, handleMovedChildren as Ft, on as G, withDefaults as Ga, queuePostRenderEffect as Gi, computed as Gn, isAsyncWrapper as Gr, resolveTransitionProps as Gt, isVaporComponent as H, watchPostEffect as Ha, pushWarningContext as Hi, checkTransitionMode as Hn, inject as Hr, patchProp as Ht, delegate as I, version as Ia, popScopeId as Ii, assertNumber as In, hydrateOnIdle as Ir, hasCSSTransform as It, renderEffect as J, withScopeId as Ja, registerRuntimeCompiler as Ji, createBaseVNode as Jn, isHydrating as Jr, shouldSetAsPropForVueCE as Jt, onBinding as K, withDirectives as Ka, rawVaporSlotKey as Ki, createAppAPI as Kn, isEmitListener as Kr, setVarsOnNode as Kt, delegateEvents as L, warn as La, popWarningContext as Li, baseEmit as Ln, hydrateOnInteraction as Lr, hydrate as Lt, defineVaporAsyncComponent as M, useTransitionState as Ma, openBlock as Mi, Teleport as Mn, toDisplayString as Mo, guardReactiveProps as Mr, ensureRenderer as Mt, defineVaporComponent as N, validateComponentName as Na, performAsyncHydrate as Ni, Text$1 as Nn, toHandlerKey as No, h as Nr, forceReflow as Nt, createTemplateRefSetter as O, useModel as Oa, onRenderTriggered as Oi, NULL_DYNAMIC_COMPONENT as On, extend as Oo, getCurrentInstance as Or, createSSRApp as Ot, defineVaporCustomElement as P, validateProps as Pa, performTransitionEnter as Pi, VaporSlot as Pn, handleError as Pr, getValue as Pt, setClassName as Q, TriggerOpTypes as Qa, resolveComponent as Qi, createElementBlock as Qn, isMemoSame as Qr, useCssVars as Qt, getDefaultValue as R, warnExtraneousAttributes as Ra, prepareTransitionLeave as Ri, baseNormalizePropsOptions as Rn, hydrateOnMediaQuery as Rr, initDirectivesForSSR as Rt, createIf as S, transformVNodeArgs as Sa, onBeforeUnmount as Si, ErrorCodes as Sn, toRefs as So, expose as Sr, VueElement as St, createPlainElement as T, useAttrs as Ta, onErrorCaptured as Ti, KeepAlive as Tn, unref as To, getAttributeMismatch as Tr, baseUseCssVars as Tt, next as U, watchSyncEffect as Ua, queueJob as Ui, cloneVNode as Un, invalidateMount as Ur, patchStyle as Ut, isFragment as V, watchEffect as Va, pushScopeId as Vi, callWithErrorHandling as Vn, initFeatureFlags as Vr, parseEventName as Vt, nthChild as W, withCtx as Wa, queuePostFlushCb as Wi, compatUtils as Wn, invokeDirectiveHook as Wr, render as Wt, setBlockKey as X, ReactiveEffect as Xa, renderSlot as Xi, createCanSetSetupRefChecker as Xn, isKeepAlive as Xr, unsafeToTrustedHTML as Xt, setAttr as Y, EffectScope as Ya, renderList as Yi, createBlock as Yn, isHydratingEnabled as Yr, svgNS as Yt, setClass as Z, TrackOpTypes as Za, resetShapeFlag as Zi, createCommentVNode as Zn, isMapEqual as Zr, useCssModule as Zt, createComponent as _, ssrUtils as _a, nextUid as _i, xlinkNS as _n, shallowReadonly as _o, devtools as _r, withVaporKeys as _t, VaporKeepAlive as a, resolveTransitionHooks$1 as aa, isTemplateRefKey as ai, vModelRadio as an, isReactive as ao, createTextVNode as ar, setProp as at, createFor as b, toHandlers as ba, onActivated as bi, Comment$1 as bn, toRaw as bo, ensureValidVNode as br, TransitionGroup as bt, VaporTransitionGroup as c, setCurrentInstance as ca, knownTemplateRefs as ci, vModelSetSelected as cn, isShallow as co, deactivate as cr, setTemplateRefBinding as ct, applyRadioModel as d, setIsHydratingEnabled$1 as da, markAsyncBoundary as di, vModelTextUpdate as dn, onWatcherCleanup as do, defineEmits as dr, template as dt, resolveDynamicComponent as ea, isRuntimeOnly as ei, useShadowRoot as en, effect as eo, createInternalObject as er, setDynamicEvents as et, applySelectModel as f, setRef as fa, matches as fi, vShow as fn, proxyRefs as fo, defineExpose as fr, txt as ft, createAssetComponent as g, ssrContextKey as ga, nextTick as gi, withModifiers as gn, shallowReactive as go, defineSlots as gr, withVaporDirectives as gt, child as h, simpleSetCurrentInstance as ha, mergeProps as hi, withKeys as hn, ref as ho, defineProps as hr, withAsyncContext as ht, VaporFragment as i, resolveTransitionChild as ia, isTemplateNode as ii, vModelDynamic as in, isProxy as io, createStaticVNode as ir, setInsertionState as it, createVaporSSRApp as j, useTemplateRef as ja, onUpdated as ji, Suspense as jn, normalizeStyle as jo, getTransitionRawChildren as jr, ensureHydrationRenderer as jt, createTextNode as k, useSSRContext as ka, onServerPrefetch as ki, SchedulerJobFlags as kn, normalizeClass as ko, getFunctionalFallthrough as kr, defineCustomElement as kt, applyCheckboxModel as l, setCurrentRenderingInstance as la, leaveCbKey as li, vModelText as ln, markRaw as lo, defineAsyncComponent as lr, setText as lt, applyVShow as m, shouldUpdateComponent as ma, mergeModels as mi, vShowOriginalDisplay as mn, readonly as mo, defineOptions as mr, vaporInteropPlugin as mt, DynamicFragment as n, resolvePropValue as na, isTeleportDeferred as ni, vModelCheckboxInit as nn, getCurrentScope as no, createRenderer as nr, setElementText as nt, VaporTeleport as o, restoreCurrentInstance as oa, isVNode as oi, vModelSelect as on, isReadonly as oo, createVNode as or, setStaticTemplateRef as ot, applyTextModel as p, setTransitionHooks as pa, mergeDefaults as pi, vShowHidden as pn, reactive as po, defineModel as pr, useVaporCssVars as pt, remove as q, withMemo as qa, registerHMR as qi, createAsyncComponentContext as qn, isFunctionalFallthroughKey as qr, shouldSetAsProp as qt, VaporElement as r, resolveTarget as ra, isTeleportDisabled as ri, vModelCheckboxUpdate as rn, getCurrentWatcher as ro, createSlots as rr, setHtml as rt, VaporTransition as s, setBlockTracking as sa, isValidHtmlOrSvgAttribute as si, vModelSelectInit as sn, isRef as so, currentInstance as sr, setStyle as st, compile as t, resolveFilter as ta, isSetEqual as ti, vModelCheckbox as tn, effectScope as to, createPropsRestProxy as tr, setDynamicProps as tt, applyDynamicModel as u, setDevtoolsHook as ua, logMismatchError as ui, vModelTextInit as un, onScopeDispose as uo, defineComponent as ur, setValue as ut, createComponentWithFallback as v, startMeasure as va, normalizeRef as vi, BaseTransition as vn, shallowRef as vo, devtoolsComponentAdded as vr, withVaporModifiers as vt, createKeyedFragment as w, useAsyncComponentState as wa, onDeactivated as wi, Fragment as wn, triggerRef as wo, flushOnAppMount as wr, baseApplyTranslation as wt, createForSlots as x, toStyleMap as xa, onBeforeMount as xi, DeprecationTypes as xn, toRef as xo, ensureVaporSlotFallback as xr, TransitionPropsValidators as xt, createDynamicComponent as y, toClassSet as ya, normalizeVNode as yi, BaseTransitionPropsValidators as yn, stop as yo, endMeasure as yr, Transition as yt, getRestElement as z, warnPropMismatch as za, prepareTransitionSwitch as zi, baseResolveTransitionHooks as zn, hydrateOnVisible as zr, nodeOps as zt };
